...
function Lt(n, s) {
  ...
  class ThinkingOrb {
    ...
    _start() {
      ...
      const preset = Lt(this.state, this.size);
      const effectiveSpeed = preset.speed * this.speed;
      ...
      this._renderFrame((now / 1000) * effectiveSpeed);

      if (this.paused) return;

      const loop = () => {
        if (!this.running || this.paused || this.destroyed) return;
        const currentNow = typeof performance !== 'undefined' && performance.now ? performance.now() : Date.now();
        const nowSec = (currentNow / 1000) * (Lt(this.state, this.size).speed * this.speed);
        this._renderFrame(nowSec);
    ...
    setState(newState) {
      if (VALID_STATES.includes(newState) && this.state !== newState) {
        ...
        const now = typeof performance !== 'undefined' && performance.now ? performance.now() : Date.now();
        const preset = Lt(this.state, this.size);
        this._renderFrame((now / 1000) * (preset.speed * this.speed));
      }
      return this;
    }
    ...
    setSpeed(newSpeed) {
      this.speed = typeof newSpeed === 'number' ? newSpeed : 1;
      return this;
    }
