/* AutoRef lib/thinking-orb.js — Zero-dependency Thinking Orb component for vanilla JS and MV3 extensions.
   Supports 9 hand-tuned animated states: 'working', 'searching', 'solving', 'listening',
   'connecting', 'weaving', 'composing', 'breathing', 'shaping'.
   Supports props: size (64 or 20), speed, dark (boolean or 'auto'), paused (boolean).
*/
(function (global) {
  'use strict';

  // --- Engine Math & Painters (Zero runtime dependencies) ---
function U(n, s, t) {
  return n + (s - n) * t;
}
function nt(n) {
...
function Lt(n, s) {
  ...
  class ThinkingOrb {
    constructor(options = {}) {
      this.state = VALID_STATES.includes(options.state) ? options.state : 'working';
      this.size = Number(options.size) || 64;
      const sp = Number(options.speed);
      this.speed = (Number.isFinite(sp) && sp > 0) ? sp : 1;
      if (options.theme !== undefined) {
        this.theme = options.theme;
        this.dark = (options.theme === 'dark') ? true : ((options.theme === 'light') ? false : 'auto');
      } else {
        this.dark = options.dark !== undefined ? options.dark : 'auto';
      }
      this.paused = !!options.paused;
      this.className = options.className || '';
      this.style = options.style || {};
      this.ariaLabel = options['aria-label'] || options.ariaLabel || null;
    ...
    _initCanvas() {
      ...
      if (typeof this.canvas.setAttribute === 'function') {
        ...
        this.canvas.setAttribute('aria-label', this.ariaLabel || STATE_LABELS[this.state] || 'AI thinking');
      }
      if (this.className) this.canvas.className = this.className;
    ...
    _syncCanvasSize() {
      ...
      const pxSize = Math.round(this.size);
      this.canvas.width = Math.round(pxSize * dpr);
      this.canvas.height = Math.round(pxSize * dpr);
      if (this.canvas.style) {
        this.canvas.style.width = pxSize + 'px';
        this.canvas.style.height = pxSize + 'px';
        this.canvas.style.display = 'inline-block';
        this.canvas.style.verticalAlign = 'middle';
        if (this.style) {
          Object.assign(this.canvas.style, this.style);
        }
      }
    }
    ...
    _isDark() {
      ...
      if (this.dark === 'dark' || this.theme === 'dark') return true;
      if (this.dark === 'light' || this.theme === 'light') return false;
      return detectDarkTheme(this.canvas);
    }

    _renderFrame(timeSec) {
      if (!this.canvas || typeof this.canvas.getContext !== 'function') return;
      const ctx = this.canvas.getContext('2d');
      if (!ctx) return;

      const dpr = Math.min(2, (typeof window !== 'undefined' && window.devicePixelRatio) || 1);
      const preset = Lt(this.state, this.size);
      const drawFn = Ct[preset.mode];
      if (!drawFn) return;

      const isDark = this._isDark();
      if (typeof ctx.setTransform === 'function') ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
      if (typeof ctx.clearRect === 'function') ctx.clearRect(0, 0, this.size, this.size);
      drawFn(ctx, this.size, timeSec, isDark, preset.opts);
    }
    ...
    _start() {
      ...
      if (this.paused) {
        this._renderFrame(this._clockSec);
        return;
      }

      if (this.isIntersecting === false || (typeof document !== 'undefined' && document.visibilityState === 'hidden')) {
        return;
      }

      const reduced = prefersReducedMotion();
      if (reduced) {
        this._renderFrame(0.6);
        return;
      }
      ...
      const loop = (timestamp) => {
        ...
        if (this._lastTimestamp != null) {
          const dt = Math.min(Math.max(0, (now - this._lastTimestamp) / 1000), 0.1);
          const preset = Lt(this.state, this.size);
          this._clockSec += dt * (preset.speed * this.speed);
        }
    ...
    _bindEvents() {
      ...
      this._visibilityHandler = () => {
        if (document.visibilityState === 'hidden') {
          this._stop();
        } else if (this.isIntersecting && !this.paused) {
          this._start();
        }
      };
      ...
      if (typeof IntersectionObserver !== 'undefined' && this.canvas) {
        try {
          this._observer = new IntersectionObserver(([entry]) => {
            ...
            if (this.isIntersecting && document.visibilityState !== 'hidden') {
    ...
    setState(newState) {
      if (VALID_STATES.includes(newState) && this.state !== newState) {
        this.state = newState;
    ...
    setTheme(theme) {
      ...
      else if (theme === 'light') this.dark = false;
      else this.dark = 'auto';
      this._renderFrame(this._clockSec);
      return this;
    }
    ...
    update(opts = {}) {
      if (opts.state !== undefined) this.setState(opts.state);
      ...
      if (opts.dark !== undefined) this.setDark(opts.dark);
      if (opts.paused !== undefined) this.setPaused(opts.paused);
      if (opts.style && this.canvas && this.canvas.style) Object.assign(this.canvas.style, opts.style);
      return this;
    }
    ...
    static resolvePreset(state, size) {
      return Lt(state, size);
    }
    ...
    static mount(container, options = {}) {
      const orb = new ThinkingOrb(options);
      if (container && typeof container.appendChild === 'function' && orb.element) {
  ...
  // Export to namespace and environments
  const NS = (global.AutoRef = global.AutoRef || {});
  NS.ThinkingOrb = ThinkingOrb;
